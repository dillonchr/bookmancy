...
const sellingStatuses = Object.keys(SELLING_STATUSES).map(
  k => SELLING_STATUSES[k]
);
  ...
  return resultSet
    .filter(
      l =>
        !!l.galleryURL &&
        l.galleryURL.length &&
        sellingStatuses.includes(l.sellingStatus[0].sellingState[0])
    )
    .map(l => {
      const [about] = l.title;
      const price = ~~parseFloat(
        l.sellingStatus[0].convertedCurrentPrice[0].__value__
      );
      const [{ shippingServiceCost }] = l.shippingInfo;
      const shipping = shippingServiceCost
        ? +shippingServiceCost[0].__value__ === 0
          ? "Free"
